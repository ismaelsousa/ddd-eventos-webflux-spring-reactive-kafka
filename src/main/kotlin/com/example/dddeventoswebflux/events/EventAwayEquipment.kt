...
import com.example.dddeventoswebflux.message.Publisher
                 ...
                 if(stopNaParada.isEmpty()){
                     ...
                     awayEquipmentRepository.getTopAwayEquipmentByRouteId(notificationMobileDTO.lastCoordinate.routeId).flatMap {away->
                         ...
                         if(
                             ...
                             if(currentDate.seconds - dateAway.seconds >= 2){ // COLOQUEI 2 E TROQUE MINUTO PARA SEGUNDOS PARA CONSEGUIR SIMULAR
                                 // throw event
                                 log.info("===============================")
                                 log.info("|    ALERTA DE PERIGO ENVIADO |")
                                 log.info("===============================\n")
                                 val eventAway = Event(null, eventType = EventType.AWAY, `when` = Date() )
                                 Mono.just(publisher.publish("${eventAway.eventType}: when ${eventAway.`when`} "))
